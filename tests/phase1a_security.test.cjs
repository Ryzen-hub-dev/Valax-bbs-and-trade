...
async function runTests() {
  ...
  const { STATIC_ALLOWED_ORIGINS, getSafeOrigin } = require("../src/config/origins.ts");
  ...
  // Test hostile / untrusted / prefix bypass origins
  ...
  function validatePrice(val) {
    return Number.isSafeInteger(val) && val > 0 && val <= 1000000;
  }
